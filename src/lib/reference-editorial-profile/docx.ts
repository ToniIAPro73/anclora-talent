...
import { normalizePdfFontName, resolveEditorialFont, DEFAULT_AVAILABLE_EDITORIAL_FONTS } from './font-normalization';
...
function styleFromBlock(block: string, availableFonts: string[]): EditorialTextStyle {
  ...
  style.paragraphSpacingBefore = twips(ppr.match(/<w:spacing\b[^>]*\bw:before="([^"]+)"/)?.[1]);
...
function styleBlock(styles: string, ...idOrNames: string[]): string | null {
  for (const idOrName of idOrNames) {
    const found = [...styles.matchAll(/<w:style\b([^>]*)>([\s\S]*?)<\/w:style>/g)].find((match) => {
      const styleId = attr(match[1], 'styleId');
      if (styleId && styleId.toLowerCase() === idOrName.toLowerCase()) return true;
      const name = match[2].match(/<w:name\b[^>]*\bw:val="([^"]+)"/)?.[1];
      if (name && name.toLowerCase() === idOrName.toLowerCase()) return true;
      return false;
    })?.[0];
...
function fallbackStyle(styles: string): string | null { return styleBlock(styles, 'Normal', 'normal', 'Párrafo estándar') ?? styles.match(/<w:docDefaults[\s\S]*?<w:rPr>([\s\S]*?)<\/w:rPr>/)?.[0] ?? null; }
...
export async function extractEditorialProfileFromDocx(buffer: Buffer | ArrayBuffer, source: DocxSourceInput): Promise<DocxEditorialAnalysis> {
  ...
  const availableFonts = DEFAULT_AVAILABLE_EDITORIAL_FONTS;
  const body = styleFromBlock(fallbackStyle(styles) ?? '', availableFonts);
  const h1 = styleBlock(styles, 'Heading1', 'Heading 1', 'Título 1', 'Encabezado 1') ? styleFromBlock(styleBlock(styles, 'Heading1', 'Heading 1', 'Título 1', 'Encabezado 1')!, availableFonts) : null;
  const h2 = styleBlock(styles, 'Heading2', 'Heading 2', 'Título 2', 'Encabezado 2') ? styleFromBlock(styleBlock(styles, 'Heading2', 'Heading 2', 'Título 2', 'Encabezado 2')!, availableFonts) : null;
  const h3 = styleBlock(styles, 'Heading3', 'Heading 3', 'Título 3', 'Encabezado 3') ? styleFromBlock(styleBlock(styles, 'Heading3', 'Heading 3', 'Título 3', 'Encabezado 3')!, availableFonts) : null;
  const h4 = styleBlock(styles, 'Heading4', 'Heading 4', 'Título 4', 'Encabezado 4') ? styleFromBlock(styleBlock(styles, 'Heading4', 'Heading 4', 'Título 4', 'Encabezado 4')!, availableFonts) : null;
  const quoteBlock = styleBlock(styles, 'Quote', 'BlockQuote', 'Block Quote', 'Cita');
  ...
  const palette = Array.from(
    new Set(
      [body.color, h1?.color, h2?.color, h3?.color, h4?.color, quote?.color].filter((c): c is string => Boolean(c)),
    ),
  );
