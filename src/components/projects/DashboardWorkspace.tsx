        ...
        {!dataAvailable ? <div className="dashboard-state" role="alert"><h2>{copy.workspaceError}</h2><p>{copy.workspaceErrorDescription}</p><button data-testid="dashboard-retry" type="button" className="dashboard-button" onClick={() => router.refresh()}>{copy.workspaceRetry}</button></div>
          ...
          : <div className={`dashboard-project-list dashboard-project-list--${layout}`}>
            ...
            >
              ...
              <div className="dashboard-project-actions"><div className="dashboard-project-counts"><span>{project.chapterCount} {copy.projectsTableChapters.toLocaleLowerCase(locale)}</span>{project.pageCount !== null && <span>{project.pageCount} {copy.projectsTablePages.toLocaleLowerCase(locale)}</span>}</div><NavigatingLink href={`/projects/${project.id}/editor`} pendingLabel={projectCopy.cardOpenEditor} className="dashboard-button dashboard-button--primary">{projectCopy.cardOpenEditor}</NavigatingLink><NavigatingLink href={`/projects/${project.id}/preview`} pendingLabel={projectCopy.cardPreview} className="dashboard-preview-link">{projectCopy.cardPreview}</NavigatingLink></div>
            </article>)}
          </div>}
