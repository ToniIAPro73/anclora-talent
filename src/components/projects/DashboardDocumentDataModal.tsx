    ...
    <div className="ac-modal dashboard-document-modal" role="dialog" aria-modal="true" aria-labelledby="dashboard-document-data-title" data-testid="dashboard-document-data-modal">
      ...
      <section className="ac-modal__panel dashboard-document-modal__panel">
        ...
        <div className="dashboard-document-modal__body">
          ...
          <section><h3>{copy.documentDataStructureHeading}</h3><div className="dashboard-document-modal__cards"><article><strong>{copy.documentDataStructureHierarchyLabel}</strong><span>{project.chapterCount} capítulos</span></article><article><strong>{copy.documentDataStructureObservedLabel}</strong><span>{reference?.observedStructure.chapterCount ? `${reference.observedStructure.chapterCount} capítulos` : copy.documentDataUnavailableValue}</span></article><article><strong>{copy.documentDataBrandHeading}</strong><span>{project.brandProfileName ?? copy.documentDataBrandNoneOption}</span></article></div></section>
        </div>
